document.addEventListener('DOMContentLoaded', () => {
  ...
  map.on('load', async () => {
    ...
    map.on('click', (e) => {
      ...
      const clickedOnLayerFeature = features.some(f => f.layer.id.startsWith('layer-') && f.layer.id !== ESTADO_LAYER_ID && f.layer.id !== ESTADO_LINE_LAYER_ID);
      ...
      if (!clickedOnLayerFeature) {
        ...
        if (isFilterActive && currentFilterSummaryHtml && infoContent) {
          infoContent.innerHTML = currentFilterSummaryHtml;
        }
      }
    });
    ...
    if (!map.listenedClicks.has(layerId)) {
      ...
      map.on('click', layerId, (ev) => {
        ...
        if (infoContent) {
          ...
          if (isFilterActive && currentFilterSummaryHtml) {
            html += `<div style="margin-top: 12px;">${currentFilterSummaryHtml}</div>`;
          }
  ...
  if (applyFilterBtn) {
    applyFilterBtn.addEventListener('click', () => {
      ...
      if (isFilterActive) {
        ...
        if (infoContent) {
          infoContent.innerHTML = `<div class="info-card"><div class="ic-label">Información</div><p style="margin: 4px 0; font-size: 0.75rem; color: var(--muted-500);">Seleccione un estado y aplique el filtro para ver los detalles.</p></div>`;
        }
